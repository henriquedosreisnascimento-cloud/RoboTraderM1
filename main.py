...
def ciclo_analise():
    ...
    while True:
        ...
        # Checa o resultado do sinal anterior (se houver)
        if ULTIMO_SINAL_CHECAR:
            checar_resultado_sinal(ULTIMO_SINAL_CHECAR)
            ULTIMO_SINAL_CHECAR = None

        print(f"[{horario_atual}] Iniciando novo ciclo de análise...")

        melhor = {'ativo': 'N/A', 'sinal': 'NEUTRO 🟡', 'score': 0, 'preco_entrada': 0.0}

        # Itera por todos os ativos para encontrar o melhor sinal
        ...
        # FILTRAGEM DE SINAIS: Só permite sinais FORTES (Score 2 ou -2)
        ...
        # Atualiza o registro do último sinal FORTE (para a caixa visual e checagem de resultado)
        ...
        # Atualiza o sinal atual (mesmo que neutro)
        ULTIMO_SINAL = {
            'horario': sinal_final['horario'],
        ...
        print(f"[{horario_atual}] 📢 Novo Sinal: {ULTIMO_SINAL['ativo']} - {ULTIMO_SINAL['sinal']} (Score: {ULTIMO_SINAL['score']})")
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            .data-item {{ margin-bottom: 10px; font-size: 1.1em; font-weight: 400; }}
            ...
            .assertividade-box p {{ margin: 0; padding: 5px 0; font-size: 1.1em; font-weight: 400;}}
            .assertividade-box span {{ font-weight: 700; color: var(--assert-borda); font-size: 3.0em; line-height: 1.1; margin: 5px 0; }}

            /* Histórico */
            h2 {{ color: var(--accent-blue); font-weight: 600; margin-bottom: 15px; }}
            pre {{ background-color: #101520; padding: 20px; border-radius: 12px; overflow: auto; color: #B0B0B0; font-size: 0.9em; }}
        ...
        <div class="container">
            ...
            <div class="main-content-grid">
                <div class="sinal-box {sinal_classe_animacao}">
                    ...
                    {'<div class="data-item">Última Análise do Robô: <strong>' + horario_exibicao + '</strong></div>' if ULTIMO_SINAL['score'] == 0 else ''}
                </div>

                <div class="assertividade-box">
                    <p>ASSERTIVIDADE RECENTE</p>
                    <p>(Últimos {assertividade_data['total']})</p>
                    <span>{assertividade_data['percentual']}</span>
                    <p>W: {assertividade_data['wins']} | L: {assertividade_data['losses']}</p>
                </div>
            </div>

            <h2>HISTÓRICO DE TRADES (Últimos {MAX_HISTORICO})</h2>
            <pre>
                {'<br>'.join([f"[{item['horario']}] {item['ativo']} -> {'<span class=win>' + item['resultado'] + '</span>' if 'WIN' in item['resultado'] else '<span class=loss>' + item['resultado'] + '</span>'} (Sinal: {item['sinal']})" for item in reversed(HISTORICO_SINAIS)])}
            </pre>

            <div class="info-box">
                <p style="font-size: 1.1em; color: var(--accent-blue); font-weight: 600;">MOTIVO DA ÚLTIMA AÇÃO:</p>
                <p>{explicacao}</p>
                <p style="margin-top: 15px;">Status do Sistema: <strong>Robô Trader M1 Rodando ✅</strong></p>
            </div>
        </div>

        <script>
            // Lógica de Alerta que é executada a cada recarregamento de 5s
            {alerta_js}
        </script>
    </body>
    </html>
...
# ====================== INÍCIO DO PROGRAMA E SERVIDOR ======================
if __name__ == '__main__':
    # A thread que roda o ciclo de análise a cada 60 segundos
    Thread(target=ciclo_analis
