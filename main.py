...
def home():
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="time-box">
                <p style="margin-bottom: 0px; font-size: 0.9em;">HORÁRIO ATUAL DE BRASÍLIA</p>
                <div class="current-time">{horario_atual_brasilia}</div>
            </div>

            <div class="warning-message" id="audio-alert">
                ⚠️ Aviso: O apito de entrada está configurado, mas o navegador pode bloqueá-lo. Clique em qualquer lugar na tela para liberar o som.
            </div>
            ...
            <div class="main-content-grid">
                <div class="sinal-box {sinal_classe_animacao}">
                    ...
                    {'<div class="data-item">Preço de Entrada: <strong>' + str(round(ULTIMO_SINAL['preco_entrada'], 5)) + '</strong></div>' if ULTIMO_SINAL['score'] != 0 else ''}
            ...
            <pre>
                {'<br>'.join([f"[{item['horario']}] {item['ativo']} -> {'<span class=win>' + item['resultado'] + '</span>' if 'WIN' in item['resultado'] else '<span class=loss>' + item['resultado'] + '</span>'} (Entrada: {item['preco_entrada']} | Expiração: {item['preco_expiracao']})" for item in reversed(HISTORICO_SINAIS)])}
            </pre>
        ...
        <script>
            // Lógica para liberar o áudio na primeira interação do usuário
            document.addEventListener('click', function() {{
                const audio = document.getElementById('alertaAudio');
                // Tenta carregar e tocar o áudio para desbloqueio
                audio.load();
                audio.play().catch(e => console.log("Tentativa de desbloqueio de áudio falhou: " + e));
                
                // Remove o aviso visual
                const alert = document.getElementById('audio-alert');
                if (alert) alert.remove();
            }}, {{ once: true }}); // Executa apenas na primeira vez
...
if __name__ == '__main__':
    # 1. A thread que roda o ciclo de análise a cada 60 segundos
    # CORREÇÃO DO ERRO DE SINTAXE AQUI!
    thread_robo = Thread(target=ciclo_analise, daemon=True) # <-- PARÊNTESE FECHADO!
    thread_robo.start()
    
    # 2. Inicia o servidor Flask (usando a porta do Render)
    logging.info(f"Iniciando servidor Flask na porta {PORT}...")
    # Quando rodando localmente (sem gunicorn), usamos app.run
